...
There are two properties that will be computed, in this tutorial:

- Band structure
- Projected density of states (PDOS)

As the goal stated, we want both: so, in Step 2.2, we select the corresponding checkboxes.
...
The calculation may take about 3 minutes to complete. While waiting, you can view information regarding the calculation by clicking the `Status` button in Step 4.

.. figure:: /_static/images/process_status.png
   :width: 12cm

You can also check the `Electronic bands + PDOS` tab (click on the `Results` button in Step 4) to see if the results are ready to be seen. As soon as some result
is available, you will be able to click on the `Load results` button.

.. figure:: /_static/images/load_results_button.png
   :width: 12cm

Results
^^^^^^^

When the calculation is finished (you may need to refresh the window), you can switch over to the `Final Geometry` (if the structure is relaxed) and `Electronic bands + PDOS` tabs to view the results.
