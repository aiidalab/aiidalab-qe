...
def get_requirements_for_ref(repo, ref):
    ...
    try:
        return tuple(
            run(
                ["git", "show", f"{ref.decode()}:requirements.txt"],
                check=True,
                capture_output=True,
                encoding="utf-8",
            ).stdout.splitlines()
...
def cli(metadata_template):
    ...
    repo = Repo(root)
    versions = {
        parse(get_version_identifier(ref.decode())): ref for ref in repo.get_refs()
    }
    requirements = {
    ...
    def get_requirements(spec, version=None):
        ...
        if version is None:
            ...
            matching_requirements = {
                requirements[version] for version in matching_versions
            }
