...
class PseudoFamilySelector(ipw.VBox):
    ...
    title = ipw.HTML("""<div style="padding-top: 0px; padding-bottom: 10px">
        <h4>Accuracy and precision</h4>
    </div>""")

    description = ipw.HTML("""<div style="line-height: 140%; padding-top: 0px; padding-bottom: 10px">
        The exchange-correlation function and pseudopotential library is set by the <b>protocol</b> configured in the "WorkChain" tab. Here you can override the defaults if desired.
    </div>""", layout=ipw.Layout(max_width='60%'))

    pseudo_family_prompt = ipw.HTML("""<div style="line-height: 140%; padding-top: 0px; padding-bottom: 10px; opacity:0.5;">
        <b><a href="https://www.materialscloud.org/discover/sssp/table/precision" 
        target="_blank">Standard Solid-state pseudopotential</a> (SSSP) protocol</b>
    </div>""")

    pseudo_family_help = ipw.HTML("""<div style="line-height: 140%; padding-top: 10px; padding-bottom: 0px; opacity:0.5;">
        If you are unsure what to choose, select 'SSSP efficiency', which for most
        ...
        expensive, but will produce even more accurate results.
    </div>""")

    dft_functional_prompt = ipw.HTML(f"""
        <div style="line-height: 140%; padding-top: 0px; padding-bottom: 10px; opacity:0.5;"><b>Exchange-correlation  functional</b> </div>
    """)

    dft_functional_help = ipw.HTML("""<div style="line-height: 140%; padding-top: 10px; padding-bottom: 10px; opacity:0.5;">
        The exchange-correlation energy is calculated based on the charge density using
        this functional. We currently provide support for two well-established generalised gradient approximation (GGA) functionals: PBE and PBEsol.
    </div>""")
    ...
    def __init__(self, **kwargs):
        ...
        self.set_pseudo_family_box = ipw.HBox(
            [self.set_pseudo_family_prompt, self.set_pseudo_family], layout=ipw.Layout(max_width="20%")
        )
        ...
        ipw.dlink(
            (self.show_ui, "value"), (self.protocol_selection, "disabled")
        )
        ipw.dlink(
            (self.show_ui, "value"), (self.dft_functional, "disabled")
        )
        ...
        super().__init__(
            children=[
                self.title,
                ipw.HBox([self.description, self.set_pseudo_family_box], layout=ipw.Layout(height="50px", justify_content='space-between')),
    ...
    def set_value_trait(self, _=None):
        self.value = (
            None if not self.set_pseudo_family.value
    ...
    def set_text_color(self, change):
        opacity = 1 if change.new else 0.5

        pattern = r'[\s\S]+opacity:([\S]+);[\S\s]+'

        for html in (
            self.pseudo_family_prompt, self.pseudo_family_help, self.dft_functional_help, self.dft_functional_prompt
        ):
            old_opacity = re.match(pattern, html.value).groups()[0]
            html.value = html.value.replace(f'opacity:{old_opacity};', f'opacity:{opacity};')        
