...
class TestWrapper:
    ...
    def test_first_time_user_message(self):
        """Test first-time-user message."""
        from pathlib import Path

        self._instansiate_mvc_components()
        assert self.view.first_time_users_infobox.layout.display == "flex"
        self.controller._on_close_first_time_info()
        assert self.view.first_time_users_infobox.layout.display == "none"
        path = Path("first-time-user")
        assert path.exists() and path.read_text().find("existing user") != -1
        path.unlink()
