...
def test_app_steps(app):
    ...
    step1.confirm()
    assert step1.state == was.State.SUCCESS
    step2 = app.steps.steps[1][1]
    assert step2.previous_step_state == was.State.SUCCESS
...
def test_app_submit(app):
    ...
    assert step3.previous_step_state == was.State.SUCCESS
    assert step3.state == step3.State.CONFIGURED
    step3.resources_config.num_cpus.value = 4
    assert step3.resources_config.num_cpus.value == 4
    builder = step3.get_builder()
    print(builder)
    # step3.submit()
